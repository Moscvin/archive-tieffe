...
@section('content')
    <div class="container-fluid spark-screen print_clienti" id="app">
        <intervention-map :chars="{{json_encode($chars)}}" :invoices_to="{{json_encode($invoicesTo)}}" :googlekey="{{ json_encode($key) }}" :pins="{{ json_encode($pins) }}" :technicianpins="{{ json_encode($technician_pins) }}" :legendpins="{{ json_encode($legend_pins) }}"></intervention-map>
    </div>
    @section('js')
