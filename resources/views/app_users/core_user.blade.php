                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <h3>Elenco degli utenti</h3>
                         @if(in_array('A',$chars))
                            <a class="btn btn-primary "  href="{{ "/app_user_add" }}"><i class="fas fa-plus"></i>&nbsp;&nbsp;Nuovo utente</a>
                         @endif
                    </div>
                    <div class="box-body">
                            ...
                            <table class="table table-responsive table-bordered table-hover table-condensed" id="users_list_table">
                                @if($pages)
                                    ...
                                    @foreach($pages as $page)
                                        <tr class="tr {{$page->isactive==1 ? '' : 'blocked'}}">
                                            <td>{{$page->username or ''}}</td>
                                            <td>{{$page->family_name or ''}}</td>
                                            <td>{{$page->name or ''}}</td>
                                            <td>{{$page->email or ''}}</td>
                                            <td>{{$page->core_groups->description or ''}} {{$page->outsider ? '(outsider)' : ''}}</td>
                                            <td class="attivo_info">{{$page->isactive==1 ? 'Si' : 'No'}}</td>
                                            @if (in_array("E", $chars))
                                            <td class="action_btn">
                                                <a href="{{ "/app_user_add" }}/{{$page->id_user}}" class="btn btn-xs btn-info" title="Modifica"><i class="fas fa-edit"></i></a>
                                            </td>
                                            @endif
                                            @if (in_array("D", $chars))
                                            <td class="action_btn">
                                            ...
                                            @endif
                                            @if (in_array("V", $chars))
                                            <td class="action_btn"><a href="{{ "/app_user_add" }}/{{$page->id_user}}/view"class="btn btn-xs btn-primary" title="Visualizza"><i class="fas fa-eye"></i></a></td>
                                            @endif
                                            @if (in_array("L", $chars))
                                            <td class="action_btn">
