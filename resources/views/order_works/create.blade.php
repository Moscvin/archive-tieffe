...
@section('content')
    {!! Form::open(array('url' => '/order_works/store', 'method' => 'post', 'class'=>'form-horizontal'))  !!}
        ...
        <div class="row tab-content">
                    ...
                    <div class="col-sm-12">
                                ...
                                @if($activeTechnicians)
                                    @foreach ($activeTechnicians as $technician)
                                        <option value="{{$technician->id_user}}"
                                                {{old('technician') == $technician->id_user ? 'selected' : ''}}
                                        >
                        ...
                        <div class="col-sm-3">
                            ...
                            <select name="hours" class="form-control">
                                <option></option>
                                <option value="0.0" {{old('hours') === '0.0' ? 'selected' : ''}}>
                                    {{'0:00'}}
                                </option>
                                @for($i = 0; $i < 12; $i++)
                                    <option value="{{($i + 0.5)}}"
                                            {{old('hours') == ($i + 0.5) ?'selected' : ''}}
                                    >
                                        {{$i . ':30'}}
                                    </option>
                                    <option value="{{($i + 1)}}"
                                            {{old('hours') == ($i + 1) ?'selected' : ''}}
                                    >
                                        {{($i + 1) . ':00'}}
                                    </option>
                                @endfor
            ...
            <div class='row form-group'>
                <div class="col-sm-4">
                    ...
                    <a class="btn btn-warning no-print" href="{{url()->previous()}}"><i class="fas fa-times"></i>&nbsp;&nbsp;Annulla</a>
                </div>
            </div>
        </div>
    </div>
