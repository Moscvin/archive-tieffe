            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        @if(in_array('A',$chars))
                            <a class="btn btn-primary "  href="{{ "/mean_add" }}"><i class="fas fa-plus"></i>&nbsp;&nbsp;Nuovo mezzo</a>
                        @endif
                    </div>
                    <div class="box-body">
                                ...
                                @if($pages)
                                    ...
                                    @foreach($pages as $page)
                                        <tr class="tr {{$page->attivo==1 ? '' : 'blocked'}}">
                                            <td>{{$page->targa or ''}}</td>
                                            <td>{{$page->marca or ''}}</td>
                                            <td>{{$page->tipologia or ''}}</td>
                                            <td class="attivo_info">{{$page->attivo==1 ? 'Si' : 'No'}}</td>
                                            @if (in_array("E", $chars))
                                            <td class="action_btn">
                                                <a href="{{ "/mean_add" }}/{{$page->id_mezzo}}" class="btn btn-xs btn-info" title="Modifica"><i class="fas fa-edit"></i></a>
                                            </td>
                                            @endif
                                            @if (in_array("D", $chars))
                                            <td class="action_btn">
                                                <button onclick="deleteItem(this)" data-my-id="{{$page->id_mezzo}}" type="button" class="action_del btn btn-xs btn-warning" title="Elimina">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            </td>
                                            @endif
                                            @if (in_array("V", $chars))
                                            <td class="action_btn"><a href="{{ "/mean_add" }}/{{$page->id_mezzo}}/view"class="btn btn-xs btn-primary" title="Visualizza"><i class="fas fa-eye"></i></a></td>
                                            @endif
                                            @if (in_array("L", $chars))
                                            <td class="action_btn">
                                                <button onclick="lockItem(this)" data-my-id="{{$page->id_mezzo}}" data-my-current="{{$page->attivo or 0}}"  type="button" class="action_block btn btn-xs btn-{{$page->attivo==1 ? 'warning' : 'primary'}}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>Sei sicuro di voler eliminare il mezzo <b>{core_item}</b>?</h4>
                </div>
...
<script>
    var lockItem = function(attr) {
        ...
        if(data_my_current > 0){
            $("#confirm_lock .modal-body h4").html("Sei sicuro di voler nascondere il mezzo <b>"+core_item+"</b>?");
            $("#confirm_lock #block-btn").text("Nascondi");
            
        } else {
            $("#confirm_lock .modal-body h4").html("Sei sicuro di voler mostrare il mezzo <b>"+core_item+"</b>?");
